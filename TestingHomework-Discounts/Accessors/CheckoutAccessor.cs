...
namespace TestingHomework_Discounts.Accessors
{
    ...
    public class CheckoutAccessor: ICheckoutAccessor
    {
        ...
        Cart_Mapper cartMapper = new Cart_Mapper();
        public Cart RedeemPromo(string code, Cart cart)
        {
            using (PromoRepository db = new PromoRepository())
            {

                var savedPromo = db.PromoCodes.Include(_promo => _promo.Product).FirstOrDefault(_dbPromo => _dbPromo.Code == code);
                var dbPromo = mapper.ModelToContract(savedPromo);
                if (dbPromo != null)
                {
        ...
        public Cart SaveCart(Cart cart)
        {
            using (PromoRepository db = new PromoRepository())
            {
                var dbModel = cartMapper.ContractToModel(cart);
